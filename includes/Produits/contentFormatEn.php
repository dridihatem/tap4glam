...
echo '<li><a href="'.urlRewrite($res_categ['titreEn'],5,$res_categ['id'],NULL,NULL,NULL).'" title="'.$res_categ['titreEn'].'" '.$class.'>'.$imm.' '.stripslashes($res_categ['titreEn']).'</a></li>';
...
echo '<div class="post-list"  style="border:0px;">
   ...
   <div class="post-txt product-txt" style="padding-top: 0px;">
      ...
      <p><a href="'.urlRewrite($res_produit['modele'],5,$res_produit['id_categorie'],$res_produit['id_sous_categorie'],$res_produit['id']).'">View detail</a>
      </p>
        ...
        <div class="have-queston marbtm50">
                        ...
                        <h3>VISIT OUR AGENT FOR FREE
                        </h3>
                        <a data-animation="animated fadeInUp" class="header-requestbtn black-request-btn hvr-bounce-to-right" href="'. urlRewrite("Ask-for-a-quote", 12, NULL, NULL, NULL).'">Ask for a quote</a>
                     </div>
...
<div class="contact-help">
                  ...
                  <a class="pdf-button" href="http://powermotors.com.tn/catalogue/catalogue_powermotors.pdf">Download the brochure</a>
                  ...
                  <div class="wdt-100 marbtm20">
                     ...
                     <div class="product-slider">
                       ...
                       <div class="product-desc">
                           ...
                           echo '<a href="'. urlRewrite($categorie->getTitreEn(), 5, $categorie->getId(), NULL, NULL).'" title="'.$categorie->getTitreEn().'">'.stripslashes($categorie->getTitreEn()).'</a>';
                           ?> -> <?php echo '<a href="'. urlRewrite($souscategorie->getTitreEn(), 5, $souscategorie->getId_categorie(), $souscategorie->getId(), NULL).'" title="'.$souscategorie->getTitreEn().'">'.$souscategorie->getTitreEn(); ?></a></li>
                         
                        </ul>
                         <h4><?php echo stripslashes($produit_detail->getModele());?></h4>
                         <h5><?php echo stripslashes($produit_detail->getTitreEn());?></h5>
                         <p class="fnt-17" style="background: #efefef;">
                             Stretcher : <?php echo $produit_detail->getChassis(); ?><br />
                             Engine : <?php  echo $produit_detail->getMoteur(); ?><br />
                             Power : <?php  echo $produit_detail->getPuissance(); ?><br />
                             Program : <?php  echo $produit_detail->getEmission(); ?><br />
                         </p>
                         <p class="line-height26">
                             <?php echo extraireTxt(formatedTxt($produit_detail->getDescriptionEn()), 100); ?>
                         </p>
                         <br/>
                         <ul class="prd-info-list">
                             <li style="width:50%; float:left;">
                                 ...
                                 if(isset($brochure) && !empty($brochure)){
                                     echo '<a href="images/ressources/'.$brochure.'" target="_blank"><i class="fa fa-file-pdf-o"></i> Download the brochure</a>';
                                 }
                                 else {echo '<a href="catalogue/catalogue_powermotors.pdf" target="_blank"><i class="fa fa-file-pdf-o"></i> Download the brochure</a>';}
                                 ?>
                             </li><li  style="width:50%; float:left;text-align: right;"><i class="fa fa-eye"></i> <?php echo $produit_detail->getVu(); ?></li></ul>
                     
                        <a style="width: 100%;" class="header-requestbtn filter-link add-cart-link hvr-bounce-to-right" href="#quotation">Request quotation</a>
                        
                       
                     </div>
                  </div>
                  <div class="wdt-100 marbtm50">
                     <h4 class="fnt20">Product description</h4>
                     <p><?php echo stripslashes($produit_detail->getDescriptionEn()); ?></p>
                  </div>
                  <div class="wdt-100">
                     <h4 class="fnt20" id="quotation">Request suotation</h4>
                     <p class="fnt-17"><strong>Model : <?php echo $produit_detail->getModele(); ?></strong></p>
...
if (!empty($captchasum) && !($captchasum == "4" || $captchasum == "quatre")) {
  ...
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Please enter 4 or four in this field.</div>';
...
<?php }
    ...
    echo ' <div class="col-lg-9 col-md-9">
         <h3>'.$souscategorie->getTitreEn().'</h3>
...
<div class="post-list">
      <span class="post-img"><img src="images/ressources/'.$res_sous_categorie['image'].'" class="img-responsive" alt="'.$res_sous_categorie['titreEn'].'"></span>
          <div class="post-txt product-txt '.$active.'">
        <a href="'.urlRewrite($res_sous_categorie['titreEn'],5,$_GET['sm'],$res_sous_categorie['id'],NULL,NULL).'" title="'.$res_sous_categorie['titreEn'].'"><h5>'.stripslashes($res_sous_categorie['titreEn']).'</h5></a>
      </div>
  </div>
...
';
    ...
    echo '<div class="block_modele" style="display:block;overflow: hidden;">';
      echo '<h4 style="font-size:23px;">Models</h4>';
...
while($res_produit = mysql_fetch_array($produit)){
    $brochure = $res_produit['fiche'];
                                 if(isset($brochure) && !empty($brochure)){
                                     $cata =  '<a class="header-requestbtn filter-link buy-now-link hvr-bounce-to-right" href="images/ressources/'.$brochure.'" target="_blank"><i class="fa fa-file-pdf-o"></i> File</a>';
                                 }
                                 else {$cata = '<a class="header-requestbtn filter-link buy-now-link hvr-bounce-to-right" href="catalogue/catalogue_powermotors.pdf" target="_blank"><i class="fa fa-file-pdf-o"></i> File</a>';}
                             
echo '<div class="shop-column blogBox">
                     <img src="images/ressources/'.$res_produit['image1'].'" class="img-responsive" alt="'.$res_produit['titreEn'].'">
                     <div class="shop-column-head">
                        <h5>Model : '.$res_produit['modele'].'</h5>
                        
                     </div>
                     <span class="shop-price">'.formatedTxt(stripslashes($res_produit['titreEn'])).'</span>
                         '.$cata.'
                     <a class="header-requestbtn filter-link buy-now-link hvr-bounce-to-right" href="'. urlRewrite($res_produit['modele'], 5, $res_produit['id_categorie'], $res_produit['id_sous_categorie'], $res_produit['id']).'" title="'.$res_produit['modele'].'">View detail</a>
                  </div>';
...
echo '<div class="shop-column blogBox">
    ...
    echo '
                ...
                while($res_categorie = mysql_fetch_array($categorie)){
                 ...
                 <a href="'. urlRewrite($res_categorie['titreEn'], 5, $res_categorie['id'], NULL, NULL).'"><img src="images/ressources/'.$res_categorie['image'].'" class="img-responsive" alt="'.$res_categorie['titreEn'].'"></a></span>
                  <div class="client-desc">
                   <span class="stitle"><a href="'. urlRewrite($res_categorie['titreEn'], 5, $res_categorie['id'], NULL, NULL).'">'.stripslashes($res_categorie['titreEn']).'</a></span>
                   <p>'. extraireTxt(stripslashes($res_categorie['descriptionEn']),200).'</p>
